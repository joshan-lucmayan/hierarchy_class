...
const NAV_ITEMS = [
  { href: "/student/home", label: "Home", icon: (active: boolean) => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={active ? "text-navy" : "text-slate-400"}>
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-gray-100 bg-white xl:hidden">
      <div className="mx-auto flex max-w-sm items-center justify-around py-2">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              {item.icon(!!active)}
              <span className={`text-[10px] font-semibold uppercase tracking-wide ${active ? "text-navy" : "text-gray-400"}`}>
                {item.label}
              </span>
