    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4 rounded-3xl border border-gray-100 bg-white p-6">
          ...
          <div className="space-y-3">
            {availableBooks.map((book) => (
              <div key={book.id} className="rounded-3xl border border-gray-100 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-semibold text-navy">{book.title}</p>
                    <p className="mt-1 text-xs text-slate-500">{book.author} · {book.genre}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => toggleBorrow(book.id)}
                    className="rounded-full border border-gray-200 bg-white px-4 py-2 text-xs font-semibold text-navy transition hover:border-navy hover:text-navy"
                  >
                    Borrow
                  </button>
                </div>
            ...
            {borrowedBooks.map((book) => (
              <div key={book.id} className="rounded-3xl border border-gray-100 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-semibold text-navy">{book.title}</p>
                    <p className="mt-1 text-xs text-slate-500">Due {book.dueDate}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => toggleBorrow(book.id)}
      ...
      <section className="rounded-3xl border border-gray-100 bg-white p-6">
        ...
        <div className="mt-4 space-y-3">
          {history.map((record) => (
            <div key={record.id} className="rounded-3xl border border-gray-100 p-4">
