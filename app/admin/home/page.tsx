    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-gray-100 bg-white p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Admin control system</p>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
        <div className="space-y-4 rounded-3xl border border-gray-100 bg-white p-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{submissions.filter((item) => item.status === "pending").length} open</span>
          </div>
          <div className="space-y-4">
            {submissions.map((submission) => (
              <div key={submission.id} className="rounded-3xl border border-gray-100 p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-navy">{submission.subject} scores</p>
                    <p className="mt-1 text-xs text-slate-500">{submission.level} • {submission.teacher}</p>
                  </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 transition hover:border-red-300 hover:text-red-700"
                  >
                    Reject
                  </button>
                </div>
        ...
        <aside className="rounded-3xl border border-gray-100 bg-white p-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">Settings</span>
          </div>
          <div className="mt-4 space-y-3">
            {rankMapping.map((mapping, index) => (
              <div key={mapping.rank} className="grid gap-3 sm:grid-cols-[1fr_1.2fr]">
                <div>
                  <p className="text-xs uppercase tracking-wider text-slate-500">{mapping.rank}</p>
