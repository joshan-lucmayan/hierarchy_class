...
- The previous **Bubblewrap TWA** implementation is archived intact at [`android-twa/`](android-twa/README.md) (same signing keystore, which the standalone app reuses; `versionName 1.26.114` / `versionCode 126114` supersedes the TWA's 1.15.90/11590 with the same applicationId and certificate, so installs upgrade cleanly).
...
- *v1.25.112:* Android profile navigation and header refinement. Profile **Achievements / Music / Photos / History** are now a compact social-profile **navigation strip** (Facebook-style UX inspiration, not its visuals): text-only tabs with a subtle gold bottom indicator on the active section, muted text when inactive, no heavy borders, no oversized segmented buttons, and only the strip scrolls on very narrow screens. The Android **collapsing header** is now truly minimal when scrolling down: the page title/label, app name, search, Florin, bell, and the header's bottom border are all **structurally removed from layout** - only the back arrow (or menu icon) bar remains, giving the content essentially fullscreen height; scrolling up restores the expanded header. Version 1.25.112 (versionCode 125112).
- *v1.26.114:* Facebook-style profile editing and story creation, Android collapsible-header rewrite, and web security headers. The **student profile** gets a unified **"Edit profile" / "Save profile"** button that toggles bio, favorite subject, and hobbies into one edit card (previously each field was edited separately), and these three fields now display inline in the hero section when not editing. A **"Create story"** button lets the profile owner pick an image, add an optional caption, and publish it to MyDay stories directly from the profile. The **Android collapsible header** is rewritten to be deterministic and flash-free: one authoritative scroll source (`document.scrollingElement`), a rAF-throttled listener, and clear hysteresis (collapse after scrolling down past 16px, expand only when back within 4px of the top) - no jitter, no duplicate listeners. The back-arrow header is now **Android-only**: web (desktop/tablet/mobile browser) student pages always show the normal full header, with navigation from the SideNav / mobile drawer. The **web deployment** now sends security headers on every response: `Content-Security-Policy` (connect-src covers Supabase REST/Realtime, Resend, PayMongo, OpenLibrary; frame-src covers YouTube/Vimeo/Spotify/SoundCloud embeds), `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, and `Referrer-Policy`. **Messenger** now shows a clear offline state ("You're offline - messages need a connection. Your draft is saved.") instead of silently failing. Version 1.26.114 (versionCode 126114).
- *v1.25.113:* Critical regression fix - the Android drawer history manipulation (`router.replace` on stale history entries, removed auto-close on route change) was leaking into the shared `MobileDrawer` component, altering web/tablet back and navigation behavior. All Android-specific history logic (`router.replace` on drawer entry, history pushState, back-arrow → menu) is now strictly gated behind `isNativeApp()`. The web drawer has been restored to its original behavior: `<Link>` + `router.push` navigation with the pathname auto-close effect. Version 1.25.113 (versionCode 125113).
