...
test("APK release metadata matches the distributed artifact contract", () => {
  ...
  assert.equal(APK_RELEASE.versionCode, 127115);
  assert.equal(APK_RELEASE.sizeBytes, 7681946);
});

test("download URL is versioned and served from the public downloads path", () => {
  assert.equal(apkDownloadUrl(), `/downloads/hierarchy-class-v1.27.115.apk`);
